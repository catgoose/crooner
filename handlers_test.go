...
func TestHandleError_NoShowDetails(t *testing.T) {
	...
	a.handleError(rec, req, 500, "test message", errors.New("underlying"))
...
func TestHandleError_ShowDetails(t *testing.T) {
	...
	a.handleError(rec, req, 400, "bad request", underlying)
...
func TestHandleError_SessionError_SetsTypeAndExtensions(t *testing.T) {
	...
	a.handleError(rec, req, 400, "Code verifier not found", &SessionError{Key: "code_verifier", Reason: ReasonNotFound})
...
func TestHandleError_AuthError_SetsTypeAndExtensions(t *testing.T) {
	...
	a.handleError(rec, req, 500, "msg", &AuthError{Op: "VerifyIDToken", Reason: "bad token", Err: nil})
...
func TestHandleError_ConfigError_SetsTypeAndExtensions(t *testing.T) {
	...
	a.handleError(rec, req, 400, "msg", &ConfigError{Field: "RedirectURL", Reason: "invalid URL", Err: nil})
...
func TestHandleError_ChallengeError_SetsType(t *testing.T) {
	...
	a.handleError(rec, req, 500, "Failed to generate state", &ChallengeError{Op: "GenerateState", Err: nil})
...
func TestHandleError_InvalidState_SetsType(t *testing.T) {
	...
	a.handleError(rec, req, 400, "Invalid state format", ErrInvalidStateFormat)
...
func TestHandleError_NonceMismatch_SetsTypeAndDetail(t *testing.T) {
	...
	a.handleError(rec, req, 400, "Nonce mismatch", ErrNonceMismatch)
...
func TestLoginHandler_ValidRedirect_StoresStateAndRedirects(t *testing.T) {
	...
	state, err := GetString(sm, req, SessionKeyOAuthState)
	if err != nil || state == "" {
		t.Errorf("session oauth_state: %v", err)
	}
	verifier, err := GetString(sm, req, SessionKeyCodeVerifier)
	if err != nil || verifier == "" {
		t.Errorf("session code_verifier: %v", err)
	}
	nonce, err := GetString(sm, req, SessionKeyOAuthNonce)
	if err != nil || nonce == "" {
...
func TestLoginHandler_InvalidRedirect_AbsoluteURL_RedirectsToSafeTarget(t *testing.T) {
	...
	_, err := GetString(sm, req, SessionKeyOAuthState)
	if err == nil {
...
func TestCallbackHandler_StateMismatch_RedirectToLogin(t *testing.T) {
	...
	_ = sm.Set(r, SessionKeyOAuthState, stateVal)
	_ = sm.Set(r, SessionKeyCodeVerifier, "verifier")
...
func TestCallbackHandler_InvalidStatePayload_RedirectToLoginURLRedirect(t *testing.T) {
	...
	_ = sm.Set(r, SessionKeyOAuthState, malformed)
	_ = sm.Set(r, SessionKeyCodeVerifier, "v")
...
func TestSetupAuth_RegistersRoutes(t *testing.T) {
	...
	// Test that the routes respond (mux pattern matching)
	for _, tc := range []struct {
		...
		// The handlers will return various status codes but should not 404
		if rec.Code == 404 {
